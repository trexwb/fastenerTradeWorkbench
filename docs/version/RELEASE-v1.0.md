...
> **状态**: ✅ 已发布（业务逻辑深度审计修复：H1 结算口径确认 / M1 匹配弹窗分配合计校验 / M2 结算删除级联发票选项与悬空标记 / M3 结算明细未定价标注；版本号 6 处统一 1.0.56，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.55
> **版本范围**: 业务逻辑深度审计（见 DELIVERY/业务逻辑深度审计报告-v1.0.55.md）发现问题的修复

---

## 业务逻辑审计修复（H1/M1/M2/M3）

### 一、H1 结算口径与状态回退不一致（🔴 高）

- **问题**：状态机允许「完成→异常」回退，但结算聚合只统计签约完成/送货中/完成——订单转异常后应收/应付从结算页消失，而收款/付款记录仍在，对账口径错位
- **修复**：`changeOrderStatus` 增加「离开结算聚合状态集」检查——订单当前处于签约完成/送货中/完成且目标状态不在其中、又存在关联结算记录时，弹确认窗提示「已关联 N 条结算记录（合计 X 元），转换后不再计入聚合（记录保留，恢复状态后重新计入）」；复用既有 `_orderLinkedSettlements`
- **不改动**：状态机本身不动（报价中→异常等既有入口行为保留）；prevStepOrder 的既有结算回退校验保留（避免双重确认——changeOrderStatus 的确认只覆盖从结算态直接转出的场景）

### 二、M1 价格库匹配多选供应商默认分配量必撞守卫（🟡 中）

- **问题**：勾选多个供应商时每个默认分配量都是全部剩余量，第二个提交必被守卫拒绝且提示「超出剩余量 0」
- **修复**：① 弹窗顶部新增实时提示「已勾选 N 家 · 分配合计 X / 剩余 Y」（超量红色）；② 勾选新供应商时其默认量自动收窄为「剩余-其他已勾选行合计」；③ 提交前校验分配合计，超量时 toast 提示**不关弹窗**（留在原处调整），替代原先「关弹窗后逐条失败」的困惑体验
- 新增 `pmUpdateHint/pmOnCheckChange/pmOnAllocInput` 三个交互函数

### 三、M2 删除结算后发票孤儿（🟡 中）

- **问题**：删除结算记录仅警告发票失配，无处理选项；孤儿发票（settleId 悬空）永久留存且无标注
- **修复**：① 删除确认弹窗新增「同时删除关联的 N 条发票记录」勾选项（级联软删进回收站）；② `syncInvoices` 对 settleId 悬空的发票自动标记 `orphan`，发票列表行显示「来源已删」标签（悬空消除时自动摘除）

### 四、M3 意向价未填的结算口径标注（🟡 中）

- **问题**：v1.0.47 起意向价可为 0/空，应收按 0 计入合法但易误解
- **修复**：`settleUnitOrderDetails` 收款分支对「有产品行但应收为 0」的订单打 `unpriced` 标记；结算明细抽屉的关联订单行显示「**未定价**」标签（hover 说明）

### 五、验证

- `node --check`（orders.js / settlements.js / invoices.js / utils.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.56；dist/sw.js 缓存名换新
- 单测（本轮此前已验证）：specMatch 8/8、mergeBatchRows 6/6
- 手测建议：① 送货中订单标记异常 → 弹结算影响确认；② 价格库匹配勾选 2 家 → 顶部提示分配合计、新勾选行默认量自动收窄；③ 合计超量提交 → 留在弹窗提示；④ 删除有关联发票的结算 → 勾选级联删除；⑤ 未填意向价的订单在结算明细中显示「未定价」

---

## v1.0.55 · ✅ 已发布

> **状态**: ✅ 已发布（恢复编辑产品/报价时选中 BOM 自动填充六维属性；版本号 6 处统一 1.0.55，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.54
> **版本范围**: 订单产品编辑弹窗 + 签约报价编辑弹窗——选中 BOM 引用时自动带出六维属性（可编辑不锁定）
...
- v1.0.45「BOM 关联仅按 SKU」调整时，把选中 BOM 后自动填充六维属性（类型/标准/直径/材质/硬度/表面处理）的逻辑一并移除了
- 用户实际使用中发现：编辑产品选了 BOM 引用后，六个属性字段全部为空，需要手动逐个选择——体验回退

### 二、修复（fillSpecFromBOM，src/views/bom.js）

- 选中 BOM 引用时，把 BOM 条目的六维属性**自动填充**到表单对应 combo
- **不锁定、可覆盖**：与 v1.0.45 前的「填充+锁死」不同，本次恢复为「填充但不锁定」——字段全程可编辑
- **不覆盖已有值**：仅当对应字段当前为空时才填充（用户已填/编辑弹窗回显的已存值不受影响）
- 填充时机仅在下拉选择回调（表单打开时不重填，v1.0.45 的防覆盖语义保留）
- 订单产品编辑（`m_` 前缀）与签约报价编辑（`ps_` 前缀）两条链路共用该函数，同步生效

### 三、验证

- `node --check`（bom.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.55；dist/sw.js 缓存名换新
- 手测建议：① 编辑产品 → 选择 BOM 引用 → 六属性自动带出（BOM 有值的字段）→ 可自由修改 → 保存；② 编辑已有产品（属性有值）→ 重选 BOM → 已有属性不被覆盖（仅空字段填充）；③ 签约报价同验证

---

## v1.0.54 · ✅ 已发布

> **状态**: ✅ 已发布（修复采购订单询货「价格库匹配」未按 SKU 过滤、显示全量报价的问题；版本号 6 处统一 1.0.54，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.53
> **版本范围**: 寻货「价格库匹配」（详情页与编辑页共用链路）的报价过滤逻辑修复
...
- `specMatch`（src/core/utils.js）此前**只按六维属性宽松匹配**（item 已填属性一致、未填属性不限制），**完全不校验 SKU**
- 又因 v1.0.45 起 BOM 关联改为「仅 SKU 关联、规格/属性独立填写」，订单行属性大多留空 → 宽松条件下几乎所有报价都命中 → 「价格库匹配」弹窗显示全量报价
- 关联链路：详情页寻货 `sourceItemFromDetail` → `buildSourceDrawerBody` → `openPriceMatchModal`（编辑页同理），两处均调 `specMatch`，单点修复全链路生效

### 二、修复后的匹配语义（SKU 优先，属性兜底防噪）

| 订单行 SKU | 报价 SKU | 结果 |
|-----------|---------|------|
| 有 | 有 | 必须完全一致（trim 容错）；不一致排除 |
| 有 | 无 | 仅当行有属性可校验且全部一致时兜底显示；行属性全空则排除（无法核验不显示） |
| 无 | 任意 | 保持既有属性宽松匹配（兼容旧流程） |

### 三、验证

- `specMatch` 单测 **8/8**：SKU 一致命中 / SKU 不同排除 / 无 SKU 报价防噪排除 / 无 SKU 报价属性一致兜底命中 / 属性不匹配排除 / 行无 SKU 兼容路径 / SKU 空格容错
- `node --check`（utils.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.54；dist/sw.js 缓存名换新
- 手测建议：订单（寻货中）→ 某产品行「寻货」→「价格库匹配」→ 仅显示与该行 SKU 一致的报价；采购订单编辑页寻货同样验证；无 SKU 报价（手动录入的）在有属性可核验时才出现

---

## v1.0.53 · ✅ 已发布

> **状态**: ✅ 已发布（六处批量导入入口统一优化交互：空内容禁用解析按钮、解析成功清空文本框、二次粘贴累加数据；版本号 6 处统一 1.0.53，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.52
> **版本范围**: 批量粘贴导入交互优化（BOM管理 / 签约报价 / 关联单位 / 采购订单 / 订单产品明细 / 寻货报价粘贴）

---

## 批量导入交互优化

### 一、三点改进（用户明确要求，六处入口统一实施）

1. **空内容禁用**：文本框无内容时「解析」按钮 disabled + 半透明 + 禁用光标；有内容实时启用（input 事件驱动）
2. **解析成功清空**：解析成功后自动清空文本框内容、禁用解析按钮（等待二次粘贴重新启用）
3. **二次粘贴累加**：再次粘贴后按钮重新启用，点击解析将新数据**追加**到已有预览（按主键去重），toast 汇总新增/跳过数
...
|------|--------|
| BOM管理 批量导入 | SKU |
| 签约报价 批量导入 | 供应商 + BOM SKU |
| 关联单位 批量导入 | 单位名称 |
| 采购订单 批量导入（整单） | 采购商 + SKU + 交货日期 |
| 订单编辑 批量粘贴产品明细 | SKU + 规格 + 数量 |
| 寻货 批量导入供应商报价 | 名称 + 规格 + 数量 + 单价 |

### 三、实现

- `src/core/utils.js` 新增三个通用函数：`bindBatchPasteUX(taId,btn)`（绑定空内容禁用交互）、`afterBatchParseOK(taId,btn)`（解析成功清空+禁用）、`mergeBatchRows(cur,incoming,keyFn)`（累加去重合并）
- 六处 `parseXxx` 统一改为：已有数组累加（`window._batchXxxData || []`）→ `mergeBatchRows` 去重合并 → 重渲染全量预览 → `afterBatchParseOK`
- 产品明细批量粘贴抽出 `renderOrderBatchPreviewAppend(allRows)` 公共渲染函数（解析与删行复用）
- 预览按钮计数从 `parsed.length`（单次）改为全量数组长度，反映累加后总数

### 四、验证

- `mergeBatchRows` 单测 **6/6**：首次全量新增、重复跳过+新追加、原数据不被覆盖、批内重复去重、复合键去重、空 incoming 幂等
- `node --check`（utils.js / bom.js / prices.js / units.js / orders.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.53；dist/sw.js 缓存名换新
- 手测建议：打开任一批量导入 → 空态时按钮灰显 → 粘贴数据按钮启用 → 解析成功文本框清空、预览出现 → 再次粘贴新数据 → 解析 → 预览累加、重复行跳过
- 2026-09-16 补充（不推进版本号，文案统一）：六处「解析」按钮文案统一为「**解析数据**」（签约报价 / 订单产品明细粘贴 / 寻货报价粘贴三处由「解析」改为「解析数据」，图标统一 icon('search','14')；BOM管理 / 关联单位 / 采购订单已是该文案）

---

## v1.0.52 · ✅ 已发布

> **状态**: ✅ 已发布（修复顶部浮动提示条（toast）相同内容整个会话只弹一次、隐藏后无法再次唤起的问题；版本号 6 处统一 1.0.52，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.51
> **版本范围**: toast 去重机制由「会话级永久去重」改为「同屏去重」（显示期间去重、消失后解除）

---

## 修复：顶部提示条（toast）隐藏后无法再次唤起

### 一、现象与根因

- **现象**：如发票管理中「发票记录不存在」的顶部提示条只出现一次，之后即使再次触发同类错误也不再显示
- **根因**：`toast()`（src/core/utils.js）内的去重集合 `_TOAST_SHOWN` 是**会话级**的——相同「类型+文案」组合首次弹出后键永久留存（直到刷新页面），后续所有相同提示在入口处被静默 return。与提示条的显示/隐藏逻辑无关，是去重键从不解除
- 该去重为 v1.0.29 时期（2026-08-28）引入，目的是抑制「数据异步恢复 / 多 render 并发触发『操作成功』连闪三四次」的同屏刷屏

### 二、修复（保留防刷屏、解除永久拦截）

- 去重集合由 `Set`（会话级）改为 `Map<key, 元素>`：相同「类型+文案」的提示**还在显示中**时不重复弹（防刷屏能力保留）；提示条淡出移除时**自动解除该键**，之后同一提示可再次正常触发
- 提示元素移除与键解除放在同一回调（TOAST_DURATION 2600ms 后淡出、TOAST_FADE 300ms 后 remove + delete），无时序窗口
- 触发链路核查：invoices.js「发票记录不存在」等所有 toast 调用方零改动

### 三、验证

- DOM 桩回归单测 4/4（抽取 utils.js 真实源码 + document 桩）：显示期间相同提示去重、不同文案不受影响、淡出后元素移除、**消失后同键可再次触发**
- `node --check`（utils.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.52；dist/sw.js 缓存名随构建换新
- 手测建议：发票管理中重复触发「发票记录不存在」（或任意同文案提示）→ 每次操作都应出现提示条；同时验证订单保存「已创建」等提示在连续保存时不会连闪

---

## v1.0.51 · ✅ 已发布

> **状态**: ✅ 已发布（签约报价列表缺省状态只保留单个「新增报价」按钮，「批量导入」入口仅在筛选生效时显示；版本号 6 处统一 1.0.51，构建已通过）
> **发布日期**: 2026-09-16
> **上一版本**: v1.0.50
> **版本范围**: 签约报价工具栏入口按筛选状态条件渲染（视觉/交互调整，批量导入功能不变）
...
- **缺省状态**（无搜索/筛选）：工具栏只保留单个「新增报价」主按钮，不再显示下拉箭头与批量导入入口
- **筛选生效**（hasPriceFilter：SKU 搜索 / 供应商 / 六维属性任一启用）：恢复完整下拉（新增报价 + 批量导入），与 BOM 管理同款样式
- 实现方式：工具栏操作区抽为 `priceActions` 条件渲染（复用既有 hasPriceFilter 判定，与空状态/计数标签同源）；批量导入功能与抽屉流程零改动

### 二、验证

- `node --check`（prices.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.51；dist/sw.js 缓存名随构建换新
- 手测建议：签约报价页缺省 → 只见「新增报价」；搜索 SKU 或选任一筛选 → 出现下拉箭头，菜单含「批量导入」
- 2026-09-16 补充（不推进版本号，同日同模块按用户反馈调整）：**废弃「筛选态才显示」的条件渲染，恢复常驻分体下拉**——「新增报价」主按钮 + 下拉箭头（更多操作）菜单含「批量导入」，缺省状态即可见，与 BOM管理/关联单位/采购订单工具栏完全统一；src/views/prices.js 的 priceActions 改回无条件渲染

---

## v1.0.50 · ✅ 已发布

> **状态**: ✅ 已发布（关联单位/采购订单的「批量导入」入口统一收入「新建」下拉按钮，样式与 BOM 管理/签约报价一致；版本号 6 处统一 1.0.50，构建已通过）
> **发布日期**: 2026-09-15
> **上一版本**: v1.0.49
> **版本范围**: 入口样式统一（视觉/交互调整，批量导入功能逻辑不变）
...
- 关联单位 / 采购订单列表页：移除独立的「批量导入」按钮，改为 BOM 管理 / 签约报价同款 **btn-group 分体下拉**——「新建」主按钮 + 下拉箭头（更多操作），菜单含「新建…」「批量导入」两项
- 下拉开合交互逐字对齐既有实现（stopPropagation + 全局单次点击关闭）；菜单 id/函数独立命名（unitDropdown / orderDropdown），互不干扰
- 操作手册第 3 章 / 第 7 章入口措辞同步（「新建」右侧下拉箭头 → 批量导入）

### 二、验证

- `node --check`（units.js / orders.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.50；dist/sw.js 缓存名随构建换新
- 手测建议：关联单位 / 采购订单列表页点「新建」右侧箭头 → 下拉出现「新建…」与「批量导入」两项；点击外部自动收起；两项功能均正常打开
- 2026-09-16 补充（不推进版本号，同日同模块按用户反馈调整）：**采购订单列表恢复独立按钮**——「批量导入」与「新建采购订单」各自独立平铺，缺省状态不再显示下拉按钮（下拉开关函数 toggleOrderDropdown/closeOrderDropdown 一并移除）；关联单位 / BOM管理 / 签约报价保持分体下拉不变。各列表入口样式现状：BOM管理=下拉、签约报价=下拉、关联单位=下拉、采购订单=独立按钮
- 2026-09-16 再补充（不推进版本号，同日同模块按用户澄清调整）：**最终统一方案确认**——四个列表（关联单位 / BOM管理 / 签约报价 / 采购订单）页面逻辑完全一致：列表缺省状态右上角只显示一个分体下拉按钮（「单个新增」主按钮 + 下拉箭头），菜单包含「单个新增」与「批量导入」两个操作；采购订单的独立按钮实验方案已撤销，恢复 v1.0.50 的下拉结构（含 toggleOrderDropdown/closeOrderDropdown）
- 2026-09-16 终版（不推进版本号）：按用户最终澄清，四个列表统一为**单个下拉按钮**——缺省状态右上角只有一个按钮（文案+内嵌小箭头，如「新建订单 ⌄」），点击按钮本体展开菜单，菜单两项：「单个新增」「批量导入」；移除原先「主按钮+独立箭头」的分体结构。采购订单按钮文案由「新建采购订单」精简为「新建订单」。涉及 bom.js / prices.js / units.js / orders.js 四处工具栏与 components.css 内嵌箭头样式
- 2026-09-16 最终定稿（不推进版本号）：撤销「四列表单按钮下拉」统一方案，回归用户明确要求的形态——**BOM管理 / 签约报价 / 关联单位 = 右上角分体下拉**（主按钮 + 箭头，菜单含同名新增与批量导入，v1.0.50 原版结构）；**采购订单 = 独立按钮**（「批量导入」btn sm + 「新建订单」btn primary 平铺，无下拉），工具栏与空态按钮文案统一「新建订单」，toggleOrderDropdown/closeOrderDropdown 移除，单按钮实验 CSS 清理
- 2026-09-16 终稿更正（不推进版本号）：按用户指示「参考关联单位/BOM管理/签约报价列表页面」——**采购订单工具栏同样对齐分体下拉**（「新建订单」主按钮 + 箭头，菜单：新建订单 / 批量导入），四个列表工具栏结构完全统一；订单空态按钮保持「新建订单」独立按钮（与三个参考列表空态一致）
- 2026-09-15 补充（文档资源，不推进版本号）：新增 `docs/导入数据Demo.xlsx`——五个批量导入入口（关联单位/BOM管理/签约报价/采购订单/订单产品明细）各一个 sheet 的示例数据 + 使用说明 sheet，数据自洽（单位/BOM SKU/采购商互相引用），并经真实解析函数回环校验 18 项通过；Demo 验证过程中发现并修复了 v1.0.49 记录的表头误判问题

---

## v1.0.49 · ✅ 已发布

> **状态**: ✅ 已发布（采购订单新增批量导入：Excel 粘贴老数据按行解析、相邻同采购商自动合并成单；版本号 6 处统一 1.0.49，构建已通过）
> **发布日期**: 2026-09-15
> **上一版本**: v1.0.48
> **版本范围**: 订单列表页新增「批量导入」抽屉（老数据迁移）；关联单位批量导入（v1.0.48）操作手册补记
...
- 订单列表页工具栏新增「批量导入」按钮（独立于订单编辑内的产品明细批量粘贴），三步流程：复制 → 粘贴解析 → 分组预览 → 批量提交
- 列序：**采购商(必填) | 项目 | 交货日期 | SKU/名称(必填) | 规格 | 数量(必填>0) | 意向价 | 报价 | 备注**；表头行自动跳过、行首序号列自动忽略
- **合并规则**：相邻且「采购商+项目+交货日期」相同的行合并为同一张订单（预览按分组展示「第 N 张」，删行后重新分组）
- 日期归一化：YYYY-MM-DD / YYYY/M/D / YYYY.M.D；无法解析置空并在预览标注（不阻断导入）
- 采购商不存在时自动新建「采购商」角色单位；订单编号统一走 genOrderNo()（PO+日期-序号）；导入订单状态一律「待确认」；SKU 与 BOM 库匹配时自动建立 SKU 关联
- 数量/价格容错：数量无效行跳过并计数；意向价/报价脏字符过滤（「1.20元」→1.2）

### 二、验证

- 解析/分组纯函数单测 9/9（表头与序号跳过、三种日期格式归一化、相邻合并与分组边界、缺采购商/SKU/数量无效计数、价格脏字符过滤）
- `node --check`（orders.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.49；dist/sw.js 缓存名随构建换新
- 手测建议：订单列表 → 批量导入 → 粘贴含表头/序号列的老数据 → 预览核对分组与新建采购商数量 → 提交后订单列表、统计卡、单位列表同步
- 2026-09-15 补充（不推进版本号，同日同类修复）：表头识别由「任一关键词命中」收紧为「≥2 个关键词命中」——修复首行数据文本偶然命中单个关键词（如采购商名称含「采购商」、备注含「交货日期」）时被误判为表头而静默丢弃的问题；关联单位批量导入（v1.0.48）同款识别逻辑同步收紧；Demo 回环校验 18 项全部通过

---

## v1.0.48 · ✅ 已发布

> **状态**: ✅ 已发布（关联单位新增批量导入：Excel 粘贴解析，参照 BOM 管理粘贴导入模式；版本号 6 处统一 1.0.48，构建已通过）
> **发布日期**: 2026-09-15
> **上一版本**: v1.0.47
> **版本范围**: 关联单位工具栏新增「批量导入」抽屉（粘贴 → 解析预览 → 批量提交）
...
- 工具栏新增「批量导入」按钮，打开批量导入抽屉（与 BOM 批量导入同款三步流程：复制 → 粘贴 → 解析预览 → 批量提交）
- 列序：**单位名称(必填) | 角色 | 联系人 | 联系人电话 | 评级 | 账期**；表头行自动跳过、行首序号列自动忽略
- 角色解析：含「供」→ 供应商、含「采」→ 采购商，双写（供应商+采购商）→ 双角色；**留空默认「供应商」**（预览中标注「默认」）
- 评级只认 主力/备选/新客；账期模糊匹配八种预设（如「月结30」→「月结30天」），无法识别置空
- 重名处理：与已有单位同名 / 批内重名的行在预览中标注「跳过」，提交时自动跳过；提交后 toast 汇总成功与跳过数量
- 记录结构与单个新建完全一致（id/name/roles/contacts/term/rating/invoice），联系人侧别按单位角色自动推导（供应/采购/双）

### 二、验证

- 解析纯函数单测 14/14（表头/序号跳过、角色双写与默认值、评级账期模糊匹配与兜底、缺名行计数、尾空列修剪）
- `node --check`（units.js）通过；`npm run vite:build` 通过；`npm run version:check` 6 处 1.0.48；dist/sw.js 缓存名随构建换新
- 手测建议：关联单位 → 批量导入 → 粘贴含表头与序号列的 Excel 数据 → 解析预览核对角色默认与重名标注 → 提交后列表与统计卡同步
...
- 2026-09-15 补充：README.md 同步更新至 v1.0.47——补记 v1.0.39–47 更新日志、修正快速开始（dist 双击入口）/项目结构（src/core|views|styles 现状）/功能模块（属性管理/BOM/签约报价/状态流转/对账结算发票）/技术特点（Vue3+Vite 构建、产物零依赖）描述，AI 助手章节补充对话附件/路径直读/知识库/多端点；文档类修改不推进版本号
...
> **状态**: ✅ 已发布（订单送货/收货锁定改为白名单：仅「送货中」可编辑；版本号 6 处统一 1.0.38，构建已通过）
> **发布日期**: 2026-09-07
> **上一版本**: v1.0.37
> **版本范围**: 采购订单「送货信息」与「收货管理」编辑权限由完成态黑名单改为白名单式——仅订单状态为「送货中」时可编辑，其它任何状态（含完成/签约完成/待确认/寻货中/报价中/异常/取消/未成交）一律只读

- 将详情视图顶部 `doneLock=o.status==='完成'`（黑名单）改为 `allowEdit=o.status==='送货中'`（白名单），`rcvLocked=!allowEdit` 统一驱动送货/收货两区
- 送货信息渲染：有已存送货信息的订单按 `allowEdit` 切换「修改送货信息」按钮与 🔒 只读锁定卡（原为仅完成态锁定）；无送货信息订单按 `allowEdit` 切换「填写发货信息」蓝卡与锁定灰卡（锁定文案：仅「送货中」状态可录入送货信息，当前状态不可修改）
- 收货管理 `receiveManageSection(o,rcvLocked)`：非「送货中」一律渲染纯文本/禁输入只读表格并显示 🔒 已锁定；底注文案改为「仅「送货中」状态可编辑收货信息，当前状态只读锁定」
- 入口兜底拦截全部白名单化：`saveDeliveryInfo` / `enterEditDelivery` 由 `status==='完成'` 改为 `status!=='送货中'` 直接 toast 拒绝；`updateReceiveField` 原完成态 + 可收状态双重拦截合并为单条白名单拦截
- AI 侧 `update_order_meta`：仅「送货中」状态允许修改交期/送货地址，其余状态返回只读错误提示
- 状态流转边界：订单流转到「送货中」即可编辑；离开「送货中」（如到完成/异常）立即锁死，回退到「送货中」后恢复可编辑；完成态仍可查看只读送货/收货信息
- 构建核验：`node --check`（orders.js / ai-tools.js）通过；`npm run vite:build` 通过；dist/sw.js 缓存名已换新为 `ftwb-v1.0.38`，产物含白名单锁定文案且旧完成态文案 0 残留
- 生效动作：Web 端需硬刷新两次破除 SW 旧缓存（stale-while-revalidate）；Tauri 桌面端需重新 `npm run tauri:build` 打包安装才生效

---

## v1.0.37 · ✅ 已发布

> **状态**: ✅ 已发布（订单完成态锁定送货信息与收货管理；内容合并记录于 v1.0.38 白名单改造）
> **发布日期**: 2026-09-07
> **上一版本**: v1.0.36
> **版本范围**: 采购订单「完成」态锁定送货信息与收货管理（黑名单式前置实现，v1.0.38 已升级为白名单式）
> **合并指向**: 内容合并记录于 **v1.0.38**（同模块送货/收货锁定连续演进，避免重复正文）

---

## v1.0.36 · ✅ 已发布

> **状态**: ✅ 已发布（AI 助手交互增强；版本号 6 处已统一，构建与打包已通过）
> **发布日期**: 2026-09-04
> **上一版本**: v1.0.35
